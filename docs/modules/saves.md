...
```ts
type SaveFile = {
  v: number; // schema version (CURRENT_SAVE_VERSION = 2)
  savedAt: number; // epoch ms
  state: GameState; // tokens, owned, shipOwned, lastTickAt
...
- Trailing ~1.5s window after **tokens / owned / shipOwned** changes (`lastTickAt`-only tick noise does not schedule; an already-pending timer is not reset).
...
- On hydrate: restore tokens + owned + shipOwned; set `lastTickAt` to now (**no offline accrual**).
...
- Soft plausibility warnings (negative tokens, unknown upgrade / ship-upgrade ids) never block play.
