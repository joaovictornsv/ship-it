...
- `src/data/contributors.ts` — opt-in skin catalog, `FAKE_DEV_NAMES`, `resolveDevSkin` / `resolveDevSkinFromPool`, talk name-drops
- `src/data/talkNames.ts` — re-exports `TALK_CONTRIBUTOR_NAMES` from the skins catalog
- `src/features/scene/DevSprite.tsx` — avatar or emoji; talk-bubble name tip on hover (contributor skins add a **Contributor** label); no outbound click; `onError` → emoji
...
| Desk kind              | Hover tip                                           | Click              |
| ---------------------- | --------------------------------------------------- | ------------------ |
| Human contributor skin | GitHub username (`skin.id`) + **Contributor** label | Same as normal Dev |
| Bot contributor skin   | GitHub username (`skin.id`) + **Contributor** label | Same as normal Dev |
| Fallback / emoji desk  | Stable fake name from pool                          | No outbound link   |
...
- Avatar 404 / decode error → that sprite switches to emoji (other desks unaffected); contributor hover rules still apply when the skin resolved.
