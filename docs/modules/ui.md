...
| Rewrite CTA   | Compact status + accent button under Ship It; **only when** Rewrite is available         | No early grayed prestige panel; Rewrites shop lives in the Rewrite dialog |
...
| `achievement-toast-out`          | `.achievement-toast-out`                       | Unlock HUD toast exit (~280ms); **off** under reduced motion                                    |
